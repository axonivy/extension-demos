...
public final class IvySamlValve extends ValveBase {
  ...
  public void invoke(Request request, Response response) throws IOException, ServletException {
    if (!requestHasActiveUserSession(request)) {
      try {
        ...
        performUserLogin(request, response, auth.getAssertion());
        sendRedirect(request, response, auth.getRedirectUri());
      } catch (AuthenticationException e) {
        response.sendError(Response.SC_FORBIDDEN, SAML_AUTH_ERROR_MSG + e.getMessage());
      }
      return;
    }
  ...
  private void performUserLogin(Request request, Response response, Assertion assertion)
  ...
  private static Optional<Cookie> getIvySessionCookie(Request request) {
    return Arrays.stream(request.getCookies()).filter(
        c -> StringUtils.equalsIgnoreCase(c.getName(), IVY_SESSION_COOKIE_NAME) && StringUtils.isNotBlank(c.getValue()))
        .findFirst();
  }
