...
public final class UserAuthenticator {
   ...
   * 
   * @return the {@link SamlUserAuthenticationResult}
   * @throws SamlValveException
   */
   ...
   *
   * @param decodedResponse the encoded response returned by the identity provider.
  ...
  private static void enforceConditions(Conditions conditions) throws AuthenticationException {
    ...
    if (now.isBefore(conditions.getNotBefore())) {
      throw new AuthenticationException("Assertion cannot be used before " + conditions.getNotBefore().toString());
    }

    if (now.isAfter(conditions.getNotOnOrAfter())) {
      throw new AuthenticationException("Assertion cannot be used after  " + conditions.getNotOnOrAfter().toString());
    }
  ...
  protected static String decodeSamlResponse(Request request) throws AuthenticationException {
    ...
    return base64SamlToken.map(token -> new String(Base64.getDecoder().decode(token), StandardCharsets.UTF_8))
        .orElseThrow(
            () -> new AuthenticationException("No saml assertion provided for request " + request.getRequestURI()));
  }
