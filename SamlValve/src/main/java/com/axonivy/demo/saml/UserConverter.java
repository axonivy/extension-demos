...
public final class UserConverter {
  ...
  public static String getIvyUserName(Request request, Assertion assertion) throws AuthenticationException {
    ...
    if (session != null) {
      ...
      if (ivyUser != null) {
        ...
        // updateIvyUser(securityContext, ivyUser, assertion);
      } else {
  ...
  private static String createIvyUser(ISecurityContext securityContext, String samlUserName, Assertion assertion,
    ...
    roles.stream().map(role -> securityContext.findRole(role)).filter(role -> role != null)
  ...
  private static List<String> getAttributeList(List<Attribute> attributes, String attributeName) {
    return attributes.stream().filter(attr -> attributeName.equals(attr.getName()))
        .map(attr -> attr.getAttributeValues()).findFirst().stream().flatMap(attrValues -> attrValues.stream())
  ...
  private static String getAttribute(List<Attribute> attributes, String attributeName) {
    return attributes.stream().filter(attr -> attributeName.equals(attr.getName()))
        .map(attr -> attr.getAttributeValues().get(0).getDOM().getFirstChild().getNodeValue()).findFirst().orElse("");
  }
}
